{
  ...
  evidence = {
    ...
    observedResult = "2026-06-30: focused mini runner verifies the SDS with explicit row-local SMS inputs without full HAT/SAT deployment. Live verifiers passed for reachability-decision, decision-reason-diagnostic, and p2p-next-hop against s-router-nixos 192.168.1.17 and s-router-clab 192.168.1.19. Reachability and decision-reason rows exposed exactly client-edge, downstream-selector, policy, upstream-selector, and core-vlan4-client-dhcp-slaac; p2p-next-hop exposed exactly router-a, downstream-selector, policy, upstream-selector, and router-b. s-router-test-clients 192.168.1.18 remained a client/substrate surface with no row router containers.";
  };
}
