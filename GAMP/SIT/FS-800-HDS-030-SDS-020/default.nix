{
  ...
  evidence = {
    ...
    '';
    observedResult = "focused network-labs source test plus CPM service-interface contract plus CLAB renderer PPPoE artifact/link test verify customer-side PPPoE records compile into explicit pppoe-handoff interfaces and render an actual Containerlab bridge link; does not claim live HAT/SAT session acceptance";
  };
}
