{
  ...
  evidence = {
    ...
    observedResult = "2026-07-21 OK at the isolated SMS-045 boundary. After every owning revision was pushed, all three lab guests were shut down together, observed offline, and returned with new boot IDs/closures, exact staged source hashes and direct network pins, and zero failed units. NixOS and CLAB both passed first-attempt IPv4/IPv6 UDP/TCP recursion through only the model-selected provider, direct-core access, local namespace sharing, deterministic lateral REFUSED behavior, denied unauthorized direct paths, persistent listeners, convergent route state, and zero reproducibility warnings. Sibling SMS rows retain their own evidence boundaries; this does not promote HAT, SAT, or production.";
  };
}
