...
let
  ...
  selectorLogicalName = target: asString ((target.logicalNode or { }).name or "");
  ...
  isSelectorTarget =
    target: isSelectorRole (selectorRole target) || isSelectorLogicalName (selectorLogicalName target);
  ...
  selectorFabricLinks = builtins.listToAttrs (
    builtins.concatLists (
      builtins.map (
        targetName:
        let
        ...
        if isSelectorTarget target && ports != { } then
          [
            {
              name = targetName;
              value = builtins.mapAttrs (_portName: port: {
  ...
  stripSelectorPorts =
    _targetName: target: if isSelectorTarget target then target // { ports = { }; } else target;
in
inventory
// {
