...
let
  ...
  getSource =
    declId:
    let
      matches = builtins.filter (s: s.declarationId == declId) sources;
    in
  ...
  getBinding =
    declId:
    let
      matches = builtins.filter (b: b.declarationId == declId) bindings;
    in
    if matches == [ ] then null else builtins.head matches;
  ...
  pathWithinRoots =
    ...
    if roots == [ ] then
      ...
      builtins.any (root: builtins.substring 0 (builtins.stringLength root) path == root) roots;
  ...
  validateDecl =
    ...
    in
    if ctx == null then
      null # Declaration does not require controlled preparation context
    else
      let
