...
| `SAT-SRC-INVENTORY-STATIC-RESERVATION` | `inventory.nix` | Static DHCP and DHCPv6 client reservation facts for a controlled NixOS client access scope, including client identity, MAC address, DHCPv6 service pool, host offsets, namespace owner, requester scope, record class, denied authority classes, and fail-closed conflict, stale-record, revocation, and fallback behavior. |
