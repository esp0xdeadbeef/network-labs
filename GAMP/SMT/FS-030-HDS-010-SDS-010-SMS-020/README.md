...
- `NETWORK_REPO_DIRECT_TEST_OK=1 bash ../network-control-plane-model/tests/FS-030-HDS-010-SDS-010-SMS-020-cpm-realization-binder-source-audit.sh` passed inside the live wrapper, proving unauthorized inventory egress and untraceable route seeded negatives are rejected while the traced binder input is accepted.
- `../network-codex-agent/scripts/smt-live-FS-030-HDS-010-SDS-010-SMS-020.sh` passed on `s-router-nixos`, `s-router-clab`, and `s-router-test-clients`.
- Evidence directory:
