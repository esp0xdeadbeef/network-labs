{
  ...
  evidence = {
    ...
    observedResult = "NOT OK: the 2026-07-18 exact-source restage proved functional provider forwarding and real bridge membership, but NixOS did not install the selected IPv6 address/default. The earlier accept_ra=2 predicate was substrate-specific and incorrect for systemd-networkd userspace RA; acceptance now requires the same functional first-boot address and selected-table default on NixOS and CLAB without prescribing a kernel sysctl value";
  };
}
