let
  ...
  maximum =
    values: builtins.foldl' (result: value: if value > result then value else result) 0 values;
  ...
  normalizeSource =
    ...
    let
      ...
      sourcePath =
        if rowSource != null && rowSource ? sourcePath then repoRoot + "/${rowSource.sourcePath}" else null;
  ...
  normalizeRow =
    ...
    let
      ...
      rawBoundary =
        row.evidenceBoundary or (if rowSource == null then null else rowSource.evidenceBoundary or null);
      constructionOnly =
        (row.evidence.isConstructionOnly or false)
        || (rawBoundary != null && builtins.match ".*construction-only.*" rawBoundary != null);
      expectedRuntimeTargets =
        if rowSource != null && rowSource ? expectedRuntimeTargets then
          builtins.attrValues rowSource.expectedRuntimeTargets
        else
          [ ];
      declaredMaximum =
      ...
      maxRuntimeTargets =
        if constructionOnly then
          0
        else if declaredMaximum != null then
          declaredMaximum
        else if expectedRuntimeTargets != [ ] then
          maximum expectedRuntimeTargets
        else
          5;
      evidenceBoundary =
        if constructionOnly then
          "construction-only"
        else if rawBoundary == null || rawBoundary == "source-stub-only" then
          "runtime"
        else
          rawBoundary;
