...
nix eval --impure --expr '
  ...
  in
    ...
    && require (builtins.elemAt topo.links 0 == [ "client-edge" "testnet-edge" ])
      "FS-310-HDS-010-SDS-010-SMS-030: link must connect client-edge and testnet-edge"
    ...
    && require (topo.nodes ? "testnet-edge" && topo.nodes."testnet-edge".role == "external")
      "FS-310-HDS-010-SDS-010-SMS-030: testnet-edge must be an external node"
    && require (topo.nodes."testnet-edge" ? external && topo.nodes."testnet-edge".external == "testnet")
      "FS-310-HDS-010-SDS-010-SMS-030: testnet-edge must declare external=testnet"
