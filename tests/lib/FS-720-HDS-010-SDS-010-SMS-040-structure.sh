...
intent_file="${repo_root}/GAMP/SMT/FS-720-HDS-010-SDS-010-SMS-040/intent.nix"
...
nix eval --impure --expr "
  let
    ...
    lab = intent.\"mini-smt\".\"FS-720-HDS-010-SDS-010-SMS-040\";
  ...
  in
    ...
    && require (lab.topology.nodes.\"client-edge\".role == \"access\")
      \"P3 FAIL: client-edge must have role access\"
    ...
    && require (relations != [ ] && (builtins.head relations).id == \"FS-720-HDS-010-SDS-010-SMS-040__mini-verify\")
      \"P5 FAIL: relation ID mismatch or empty relations\"
    && require (builtins.any (r: r.action == \"allow\" && r.from.name == \"client\" && r.to.uplinks or [] != []) relations)
      \"P6 FAIL: client-to-uplink allow relation missing\"
...
echo "SMS predicate verification for FS-720-HDS-010-SDS-010-SMS-040 is in the owning repo construction test."
