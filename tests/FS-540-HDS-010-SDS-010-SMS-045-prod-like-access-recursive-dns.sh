...
jq -e --arg trace "${trace_id}" --arg internet_relation "${internet_relation_id}" '
  ...
  def hasRuntimeOriginRule($site; $target; $from; $to):
    [ ($site.runtimeTargets[$target].forwardingIntent.rules // [])[]
      | select(
          ...
          and .toInterface == $to
          and ((prefixes(.) | index("10.54.45.1/32")) != null)
        )
    ] | length == 1;
  def hasCoreDnsEgressRule($site; $coreTarget):
    [ ($site.runtimeTargets[$coreTarget].forwardingIntent.rules // [])[]
      | select(
          ...
          and .trafficType == "dns"
          and .family == 4
          and ((prefixes(.) | index("10.54.45.1")) != null)
          and (.matches == [
  ...
  | if $targets != $expectedTargets then
    ...
    elif (($site.ipv4.internetModes.privateNat44[0].sourcePrefixes // []) | index("10.54.45.0/24")) == null then
    ...
    elif (($site.hostNat.hostMasqueradePrefixes4 // []) | index("10.54.45.0/24")) == null then
    ...
    elif ($accessDns.outgoingInterfaces != ["10.54.45.1"]) then
      error("access DNS must use the tenant gateway as its IPv4 recursive source")
    elif ($accessDns.roles.recursion.outgoingInterfaces != ["10.54.45.1"]) then
    ...
    elif ($runtimeOriginEgress.preferredSources.ipv4 != "10.54.45.1") then
      error("access DNS runtimeOriginEgress must prefer 10.54.45.1")
    elif ([($runtimeOriginEgress.sourcePrefixes // [])[] | select(.family == 4 and .prefix == "10.54.45.1/32")] | length) != 1 then
      error("access DNS runtimeOriginEgress must carry 10.54.45.1/32")
      ...
      error("core must allow IPv4 DNS service egress from 10.54.45.1 to VLAN4")
    elif ([($site.endpointAssignment // {}) | keys[] | select(. == "prod-like-dns-client01" or . == "prod-like-dns-clab-client01")] | length) != 0 then
