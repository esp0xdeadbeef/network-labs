...
# lower-layer fixtures only and are not SAT source evidence by themselves.
{
  esp = {
    ...
    nixos = {
      ...
      # SAT-SRC-INTENT-NIXOS-UPSTREAM-EMULATION: NixOS SAT behavior source
      # for the emulated ISP. The customer uplink obtains public-facing IPv4
      # and IPv6 only through PPPoE, not WAN DHCP or SLAAC.
      upstreamEmulation = {
        emulatedIsp = {
    ...
    clab = {
      ...
      # SAT-SRC-INTENT-CLAB-UPSTREAM-EMULATION: CLAB SAT behavior source for
      # the emulated ISP. Runtime packaging may differ from NixOS, but the
      # modeled upstream handoff remains PPPoE-only.
      upstreamEmulation = {
        emulatedIsp = {
