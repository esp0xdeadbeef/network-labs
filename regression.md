...
- state=still-broken | target=s-sigma-hetz-underlay-access-egress-intent | evidence=2026-05-23 full rebuild-loop network-* sweep failed in `network-compiler` with `E_OVERLAY_UNDERLAY_ACCESS_WAN_EGRESS_REQUIRED`: overlay `east-west` underlayAccess tenant `iot` has no allowed egress relation to the underlay target external at `transport.overlays.east-west.underlayAccess` for `esp.hetz`. | reason=The compiler is correctly refusing an underlay access tenant that cannot reach the external underlay. The lab intent must select a tenant/access path with modeled WAN egress for the overlay daemon underlay, not an isolated or payload-dependent tenant.
