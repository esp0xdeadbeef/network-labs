...
- state=still-broken | target=s-sigma-nebula-underlay-dedicated-access | evidence=2026-05-24 live `s-router-test` still rendered `nixos-router-core-nebula` with `access-client@if69 10.10.0.3/31` plus `upstream@if68 10.10.0.18/31`; `ip route get 1.1.1.1` selected `via 10.10.0.2 dev access-client src 10.19.0.8`, and the public ping failed. `gron /etc/network-artifacts/compiler.json | rg 'core-nebula|access-client|underlay|p2p|10.10.0.(2|3|18|19)'` shows the bad shape already in compiler provenance: `esp.nixos.topology.links[2] = nixos-router-core-nebula -> nixos-router-access-client`, `esp.nixos.topology.links[3] = nixos-router-core-nebula -> nixos-router-upstream`, and matching CLAB links. The same artifact also has `transport.overlays[0].underlayAccess.kind = "tenant"; name = "client"`, proving the semantic selector exists but the lab still gives the pipeline a p2p router link. | reason=The underlay/bootstrap side of a Nebula core must be a host-like client attachment on the selected access tenant, with IPv4 learned by DHCP and IPv6 by SLAAC/RA. It must not be a generated `/31` p2p topology link to `access-client`, and it must not have a separate direct `core-nebula -> upstream` adjacency for the daemon underlay. The owning fix starts in `labs/lab-s-sigma/s-router-test-three-site/{intent.nix,inventory.nix}` and may require compiler/NFM support if the current schema cannot express a core node as a DHCP/SLAAC client on an access tenant bridge.
...
- state=still-broken | target=s-sigma-core-nebula-underlay-attachment-shape | evidence=2026-05-24 live `s-router-test` no longer shows the old host WAN bridge on core-nebula, but it replaced that with another wrong shape: a p2p `/31` interface to `nixos-router-access-client`. The current compiler provenance and inventory realization still contain `underlay-core-nebula` / `underlay-access-client` ports backed by `p2p-*-access-client-*-core-nebula` links for NixOS, Hetzner, and CLAB. | reason=Removing the hidden WAN bridge was not enough. The access-side underlay must be an ordinary tenant/client LAN attachment, not another p2p router adjacency. Tests must reject any `core-nebula <-> access-client` p2p link or inventory port named `underlay-*core-nebula` and require DHCPv4 plus SLAAC/RA on the selected access tenant bridge.
...
- state=still-broken | target=s-sigma-nebula-underlay-access-inventory-realization | evidence=2026-05-24 the previous "fixed-locally" inventory realization is the live bug. `nixos-router-core-nebula` came up with `access-client@if69 10.10.0.3/31`, while `nixos-router-access-client` had reciprocal `core-nebula@if76 10.10.0.2/31` plus the real tenant LAN `tenant-client@if77 10.20.20.1/24`. That proves inventory realized a router p2p instead of placing core-nebula on the tenant-client LAN. | reason=Inventory may bind bridges, interface names, MACs, MTU, VLANs, and DHCP/SLAAC realization facts, but it must not turn an overlay daemon underlay access selection into a p2p fabric link. The next test should fail if any `{nixos,hetz,clab}-router-core-nebula` underlay port has a `link = "p2p-*access-client*core-nebula"` or `/31` addressing; it should require host-like client attachment semantics on the selected tenant.
