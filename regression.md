...
- state=fixed-locally | `s-router-overlay-dns-lane-policy` and the prod-like
...
- state=fixed-locally | `tests/test-hostile-exits-east-west-only.sh` now guards both sides of that
...
- state=fixed-locally | `examples/s-router-test-three-site` was moved to
...
- state=fixed-locally | `tests/test-readable-examples-and-labs.sh` rejects `builtins.fromJSON` blobs
...
- state=fixed-locally | `tests/test-lab-runtime-secret-boundary.sh` rejects public IPv4/GUA IPv6,
...
- state=fixed-locally | Lab runtime staging entrypoints exist for `getCompilerInput`, `getInventory`,
  `getInventorySops`, and `getResolvedInventory`.
- state=fixed-locally | `getResolvedInventory` now resolves the lab DNS forwarder placeholders from
...
- state=fixed-locally | `runtimeNodes.*.unsafeRoutes` was removed from the s-sigma lab inventories.
  ...
  contracts.
- state=fixed-locally | The stale `nebula-core` runtime node was removed from the s-sigma lab and the
...
- state=fixed-locally | `examples/ipv6-pd-downstream-delegation` covers a provider /48, a normal
  ...
  through CPM.
- state=fixed-locally | `tests/test-s-router-client-bridge-contract.sh` now rejects
  ...
  NixOS lab inventory.
- state=fixed-locally | Inventory no longer carries synthetic `containers.default` bindings for every
...
- state=fixed-locally | Service provider endpoint realization is explicit for the overlay/DNS examples
...
- state=broken | The 2026-05-13 fast live refresh
...
- state=broken | `./tests/test.sh` is currently blocked by its pinned downstream CPM/NFM chain,
  ...
  `network-labs`.
- state=broken | The prod-like s-sigma lab still uses one shared `inventory-clab.nix` alongside
...
- state=next-debugging-target | Add focused `network-labs` tests that compile both prod-like lab targets at
...
- state=next-debugging-target | After `network-labs` tests pass, continue down the locked chain in order:
