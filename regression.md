...
- state=fixed-but-only-locally-tested | target=s-sigma-core-nebula-underlay-attachment-shape | evidence=2026-05-24 the focused regression now rejects the exact stale inventory shape: any `underlay-*core-nebula` / `underlay-access-client` port, any `core-nebula` upstream p2p port, any `p2p-*-access-client-*-core-nebula` or `p2p-*-core-nebula-*-upstream` link binding, and any matching stale bridge on the NixOS, Hetzner, or CLAB host inventories. The current inventory realizes each Nebula core through a `tenant-client` port on the `client` bridge with `logicalInterface = "tenant-client"`, and `NETWORK_REPO_DIRECT_TEST_OK=1 bash tests/test-lab-sigma-nebula-underlay-access.sh` passes. | reason=Removing the hidden WAN bridge was not enough. The access-side underlay must be an ordinary tenant/client LAN attachment, not another p2p router adjacency. Live rebuild-loop validation must still prove the rendered runtime gets DHCPv4/SLAAC instead of static p2p addressing.
